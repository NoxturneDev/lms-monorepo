...
// ============================================
// RPC: At-Risk Students (per-course)
// ============================================
...
func (s *server) ReconcileGradesSync(ctx context.Context, req *statspb.ReconcileSyncRequest) (*statspb.ReconcileSyncResponse, error) {
	...
	type gradeToFix struct {
		id           string
		assignmentID string
	}

	var pendingFixes []gradeToFix
	for rows.Next() {
		var g gradeToFix
		if err := rows.Scan(&g.id, &g.assignmentID); err != nil {
			return nil, status.Errorf(codes.Internal, "scan error: %v", err)
		}
		pendingFixes = append(pendingFixes, g)
	}

	var fixedCount int32
	var unfixableCount int32

	for _, g := range pendingFixes {
		...
		if err != nil || !assignmentCategory.Valid {
			unfixableCount++
			continue
		}
		...
		_, err = s.db.ExecContext(ctx,
			...
			assignmentCategory.String, g.id,
		)
		if err != nil {
			log.Printf("Failed to update grade %s with category: %v", g.id, err)
			unfixableCount++
		} else {
			fixedCount++
			log.Printf("Fixed: Grade %s now has category: %s", g.id, assignmentCategory.String)
		}
	}

	return &statspb.ReconcileSyncResponse{
		CourseId:              req.CourseId,
		GradesFixed:           fixedCount,
		InconsistenciesFound:  unfixableCount,
...
// ============================================
// RPC: Early Warning System (Decision Tree)
// ============================================
...
func (s *server) GetStudentsAtRisk(ctx context.Context, req *statspb.StudentsAtRiskRequest) (*statspb.StudentsAtRiskResponse, error) {
	// Step 1: Collect class averages per course first (fully consume before next query)
	...
	// Step 2: Get per-student per-course averages
	...
	var criticalCount, warningCount int32

	for rows.Next() {
		var studentID, courseID string
		var avgPercentage float64

		if err := rows.Scan(&studentID, &courseID, &avgPercentage); err != nil {
			return nil, status.Errorf(codes.Internal, "scan error: %v", err)
		}
		...
		// Decision Tree:
		// CRITICAL: average score < 60%
		// WARNING: average score < class average - 15 points
		if avgPercentage < 60 {
			riskLevel = "CRITICAL"
			reason = fmt.Sprintf("Average score %.2f%% is below 60%%", avgPercentage)
			criticalCount++
		} else if avgPercentage < classAvg-15 {
			riskLevel = "WARNING"
			reason = fmt.Sprintf("Average %.2f%% is 15+ points below class average %.2f%%", avgPercentage, classAvg)
			warningCount++
		}
...
func (s *server) ForecastEnrollment(ctx context.Context, req *statspb.EnrollmentForecastRequest) (*statspb.EnrollmentForecastResponse, error) {
	...
	if len(years) < 2 {
		return nil, status.Errorf(codes.FailedPrecondition, "need at least 2 years of historical data, found %d", len(years))
	}
	...
	// Determine trend once (same for all projections since slope is constant)
	trend := "STABLE"
	...
	for i := 1; i <= int(forecastYears); i++ {
		...
		predicted := regression.Slope*float64(forecastYear) + regression.Intercept
		if predicted < 0 {
			predicted = 0
		}
		...
		projections = append(projections, &statspb.EnrollmentProjection{
			...
			ProjectedStudents: int32(predicted),
			ConfidenceLevel:   confidence,
			Trend:             trend,
		})
	}

	// Determine forecast accuracy label
	accuracy := "MEDIUM"
...
// ============================================
// NIGHTLY RISK DETECTION JOB
// ============================================

func (s *server) runNightlyRiskDetection() {
	log.Println("[Risk Detection] Running nightly early warning scan...")

	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()

	resp, err := s.GetStudentsAtRisk(ctx, &statspb.StudentsAtRiskRequest{})
	if err != nil {
		log.Printf("[Risk Detection] Error: %v", err)
		return
	}

	if len(resp.AtRiskStudents) == 0 {
		log.Println("[Risk Detection] No at-risk students found")
		return
	}

	// Upsert results into insight_student_risks table
	for _, risk := range resp.AtRiskStudents {
		_, err := s.db.ExecContext(ctx, `
			INSERT INTO insight_student_risks (student_id, course_id, risk_level, warning_reason, current_average, class_average, detected_at, updated_at)
			VALUES ($1, $2, $3, $4, $5, $6, NOW(), NOW())
			ON CONFLICT (student_id, course_id, risk_level)
			DO UPDATE SET warning_reason = $4, current_average = $5, class_average = $6, updated_at = NOW()
		`, risk.StudentId, risk.CourseId, risk.RiskLevel, risk.WarningReason, risk.CurrentAverage, risk.ClassAverage)
		if err != nil {
			log.Printf("[Risk Detection] Failed to upsert risk for student %s: %v", risk.StudentId, err)
		}
	}

	log.Printf("[Risk Detection] Complete: %d critical, %d warning", resp.TotalCritical, resp.TotalWarning)
}
...
func main() {
	...
	// Periodic reconciliation job (every 30 seconds)
	go func() {
	...
	// Nightly risk detection job (every 24 hours, runs once on startup too)
	go func() {
		// Run once on startup after a short delay
		time.Sleep(10 * time.Second)
		srv.runNightlyRiskDetection()
